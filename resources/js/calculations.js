(function($) {
    ...
    $(document).ready(function() {
        ...
        $('.var_img_source').hover(function() {
            ...
            alert(logo) // get logo from data-icon parameter
        ...
        function get_price_accordint_to_sku() {
            var value = $("input[name='attr_val_name_modal[]']").map(function() {
                return $(this).val();
            }).get();
            var id = $("input[name='attr_val_id_modal[]']").map(function() {
                return $(this).val();
            }).get();
            var product_id = $("#product_id_modal").val();
            var user_id = $('#seller_id_modal').val();
            $('#pre-loader').show();
            $.post('seller/get-seller-product-ku-wise-price.html', {
                _token: 'tW3d9kIg46N8C3388j7rbEM4f6NWVGwGrgFxQP8i',
                id: id,
